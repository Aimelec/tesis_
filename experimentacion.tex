...
Por otro lado, SWOW-RP es una base de datos resultante de varias tareas de asociación de palabras \bb{[aclarar que está hecho sobre hablantes roioplatenses. Poner alguna cita]}, 
donde a partir de una palabra inicial (\textit{cue}), los sujetos contestaban con las primeras tres 
palabras que se les venían a la mente. \bb{[Elimino salto de párrafo]} A partir de ahí, el repositorio brinda información ya procesada sobre estas tareas, ya sea 
...
Previo a este trabajo, dentro del laboratorio de la mano del %licenciado
\bb{doctorando} Fermin Travi se estuvo 
investigando sobre la incidencia de la información cognitiva sobre otro tipo de modelos previos 
a la arquitectura AWD-LSTM, en este caso, Word2Vec.\bb{[Elimino salto de párrafo]} Haciendo hincapié en el mismo, se utilizó una arquitectura de Word2Vec basada en \textit{skip-grams} y 
se entrenó al modelo utilizando como corpus base el mismo corpus aprovechado en este trabajo, 
valiéndose del 100\% del mismo durante el entrenamiento por una cantidad de 5 épocas. \bb{[Elimino salto de párrafo]}
...
del repositorio SWOW-RP \bb{(Figura \ref{fig:swow_word2vec})}.
\bb{[Elimino salto de párrafo]} Como se puede observar, los modelos reentrenados con los \textit{scanpaths} muestran una peor 
...
\subsection{\bb{Exploración de hiperparámetros}}
\subsubsection{\bb{Tamaño del Corpus y Épocas}}
...
\begin{figure}[htb]
    ...
    \end{subfigure}
    \caption{\bb{Cuidado que los índices que usas acá no se corresponden con los de la figura (acá están en mayus y en la figura en minuscula)}En A, correlación con Multi-Simlex a lo largo de las épocas para el modelo con 
...
\bb{En síntesis, observamos que} reducir el tamaño del corpus y aumentar la cantidad de épocas mejoró ligeramente la 
...
\subsubsection{\bb{Learning Rate}}
...
\subsubsection{\bb{Variación del Learning Rate}}
...
los resultados sobre SWOW-RP por parte de los modelos de Word2Vec (ver \bb{Sección} \ref{sec:trabajos_previos}), al igual que en el 
